import axios from 'axios';
import stockCodesData from '../data/stockCodes.json';

// KIS API 설정
const KIS_APP_KEY = process.env.KIS_APP_KEY || '';
const KIS_APP_SECRET = process.env.KIS_APP_SECRET || '';
const KIS_ACCOUNT_NO = process.env.KIS_ACCOUNT_NO || '';
const KIS_IS_MOCK = process.env.KIS_IS_MOCK === 'true';

// API Base URL (모의투자 vs 실투자)
const BASE_URL = KIS_IS_MOCK
    ? 'https://openapivts.koreainvestment.com:29443'
    : 'https://openapi.koreainvestment.com:9443';

// 종목코드 매핑 (JSON 파일에서 로드)
const STOCK_CODE_MAP: Record<string, string> = stockCodesData as Record<string, string>;

// 토큰 캐시
let accessToken: string | null = null;
let tokenExpireTime: number = 0;

// OAuth 토큰 발급
export async function getAccessToken(): Promise<string> {
    // 캐시된 토큰이 유효하면 재사용
    if (accessToken && Date.now() < tokenExpireTime - 60000) {
        return accessToken;
    }

    try {
        const response = await axios.post(`${BASE_URL}/oauth2/tokenP`, {
            grant_type: 'client_credentials',
            appkey: KIS_APP_KEY,
            appsecret: KIS_APP_SECRET,
        });

        accessToken = response.data.access_token;
        // 토큰 만료시간 설정 (보통 24시간, 여유를 두고 23시간으로)
        tokenExpireTime = Date.now() + 23 * 60 * 60 * 1000;

        console.log('✅ KIS API 토큰 발급 완료');
        return accessToken!;
    } catch (error: any) {
        console.error('❌ KIS API 토큰 발급 실패:', error.response?.data || error.message);
        throw new Error('KIS API 토큰 발급 실패');
    }
}

// 종목명으로 종목코드 조회
export function getStockCode(stockName: string): string | null {
    return STOCK_CODE_MAP[stockName] || null;
}

// 현재가 조회 결과 타입
export interface StockPrice {
    stockCode: string;
    stockName: string;
    currentPrice: number;      // 현재가
    changePrice: number;       // 전일대비
    changeRate: number;        // 등락률
    volume: number;            // 거래량
    high: number;              // 고가
    low: number;               // 저가
    open: number;              // 시가
}

// 국내주식 현재가 조회
export async function getStockPrice(stockCode: string): Promise<StockPrice | null> {
    try {
        const token = await getAccessToken();

        const response = await axios.get(
            `${BASE_URL}/uapi/domestic-stock/v1/quotations/inquire-price`,
            {
                headers: {
                    'Content-Type': 'application/json; charset=utf-8',
                    'authorization': `Bearer ${token}`,
                    'appkey': KIS_APP_KEY,
                    'appsecret': KIS_APP_SECRET,
                    'tr_id': KIS_IS_MOCK ? 'FHKST01010100' : 'FHKST01010100',
                },
                params: {
                    FID_COND_MRKT_DIV_CODE: 'J',  // 시장구분 (J: 주식)
                    FID_INPUT_ISCD: stockCode,    // 종목코드
                },
            }
        );

        const data = response.data.output;

        if (!data || response.data.rt_cd !== '0') {
            console.error(`주가 조회 실패: ${stockCode}`, response.data);
            return null;
        }

        return {
            stockCode,
            stockName: data.hts_kor_isnm || '',
            currentPrice: parseInt(data.stck_prpr) || 0,
            changePrice: parseInt(data.prdy_vrss) || 0,
            changeRate: parseFloat(data.prdy_ctrt) || 0,
            volume: parseInt(data.acml_vol) || 0,
            high: parseInt(data.stck_hgpr) || 0,
            low: parseInt(data.stck_lwpr) || 0,
            open: parseInt(data.stck_oprc) || 0,
        };
    } catch (error: any) {
        console.error(`주가 조회 에러 (${stockCode}):`, error.response?.data || error.message);
        return null;
    }
}

// 여러 종목 현재가 일괄 조회 (순차 호출, rate limit 고려)
export async function getMultipleStockPrices(stockCodes: string[]): Promise<Map<string, StockPrice>> {
    const results = new Map<string, StockPrice>();

    for (const code of stockCodes) {
        const price = await getStockPrice(code);
        if (price) {
            results.set(code, price);
        }
        // API rate limit 방지 (초당 20건 제한)
        await new Promise(resolve => setTimeout(resolve, 60));
    }

    return results;
}

// 종목명 목록으로 현재가 조회
export async function getStockPricesByNames(stockNames: string[]): Promise<Map<string, StockPrice>> {
    const results = new Map<string, StockPrice>();

    for (const name of stockNames) {
        const code = getStockCode(name);
        if (!code) {
            console.log(`종목코드 없음: ${name}`);
            continue;
        }

        const price = await getStockPrice(code);
        if (price) {
            price.stockName = name;  // 원래 이름으로 덮어쓰기
            results.set(name, price);
        }

        // API rate limit 방지
        await new Promise(resolve => setTimeout(resolve, 60));
    }

    return results;
}

// ==================== 해외주식 API ====================

// 해외주식 현재가 조회 결과 타입
export interface OverseasStockPrice {
    symbol: string;
    name: string;
    currentPrice: number;      // 현재가 (USD)
    changePrice: number;       // 전일대비
    changeRate: number;        // 등락률
    volume: number;            // 거래량
    high: number;              // 고가
    low: number;               // 저가
    open: number;              // 시가
    prevClose: number;         // 전일종가
}

// 거래소 코드 매핑
const EXCHANGE_CODE_MAP: Record<string, string> = {
    'NASDAQ': 'NAS',
    'NYSE': 'NYS',
    'AMEX': 'AMS',
    'OTC': 'OTC',
};

// 해외주식 현재가 조회
export async function getOverseasStockPrice(
    symbol: string,
    exchange: string = 'NASDAQ'
): Promise<OverseasStockPrice | null> {
    try {
        const token = await getAccessToken();
        const exchangeCode = EXCHANGE_CODE_MAP[exchange] || 'NAS';

        const response = await axios.get(
            `${BASE_URL}/uapi/overseas-price/v1/quotations/price`,
            {
                headers: {
                    'Content-Type': 'application/json; charset=utf-8',
                    'authorization': `Bearer ${token}`,
                    'appkey': KIS_APP_KEY,
                    'appsecret': KIS_APP_SECRET,
                    'tr_id': 'HHDFS00000300',  // 해외주식 현재가 조회
                },
                params: {
                    AUTH: '',
                    EXCD: exchangeCode,       // 거래소 코드
                    SYMB: symbol,             // 종목코드 (티커)
                },
            }
        );

        const data = response.data.output;

        if (!data || response.data.rt_cd !== '0') {
            console.error(`해외주가 조회 실패: ${symbol}`, response.data.msg1 || response.data);
            return null;
        }

        return {
            symbol,
            name: data.rsym || symbol,
            currentPrice: parseFloat(data.last) || 0,
            changePrice: parseFloat(data.diff) || 0,
            changeRate: parseFloat(data.rate) || 0,
            volume: parseInt(data.tvol) || 0,
            high: parseFloat(data.high) || 0,
            low: parseFloat(data.low) || 0,
            open: parseFloat(data.open) || 0,
            prevClose: parseFloat(data.base) || 0,
        };
    } catch (error: any) {
        console.error(`해외주가 조회 에러 (${symbol}):`, error.response?.data || error.message);
        return null;
    }
}

// 여러 해외 종목 현재가 일괄 조회
export async function getMultipleOverseasStockPrices(
    stocks: { symbol: string; exchange: string }[]
): Promise<Map<string, OverseasStockPrice>> {
    const results = new Map<string, OverseasStockPrice>();

    for (const stock of stocks) {
        const price = await getOverseasStockPrice(stock.symbol, stock.exchange);
        if (price) {
            results.set(stock.symbol, price);
        }
        // API rate limit 방지 (초당 20건 제한)
        await new Promise(resolve => setTimeout(resolve, 60));
    }

    return results;
}
