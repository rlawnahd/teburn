...
const THEME_CATEGORIES: Record<string, { label: string; keywords: string[] }> = {
    ...
    battery: {
        label: '2차전지',
        keywords: ['2차전지', '배터리', '리튬', '전고체', '양극재', '음극재', '전해액', '분리막', '폐배터리'],
    },
    bio: {
        label: '바이오',
        keywords: [
    ...
    energy: {
        label: '에너지',
        keywords: ['원전', '신재생', '태양광', '풍력', '수소', '전력기기', 'SMR', '에너지저장', 'ESS'],
    },
...
function ThemeFlipCard({ theme, onClick }: { theme: MergedThemeData; onClick: () => void }) {
    ...
    return (
        <div onClick={onClick} className="group cursor-pointer h-[140px]" style={{ perspective: '1000px' }}>
            <div
                        ...
                        <div className="flex flex-col h-full text-xs">
                            ...
                            {leaderStock && (
                                <div className="flex items-center gap-1.5 mb-2">
                                    ...
                                    <span className="text-[var(--text-secondary)] font-medium truncate flex-1">
                                        {leaderStock.stockName}
                                    </span>
                                    <span
                                        className={`font-bold flex-shrink-0 ${
                                            leaderStock.changeRate > 0
                                                ? 'text-[var(--rise-color)]'
                                                : 'text-[var(--fall-color)]'
                                        }`}
                                    >
                                        {leaderStock.changeRate > 0 ? '+' : ''}
                                        {leaderStock.changeRate.toFixed(1)}%
                                    </span>
                                </div>
                            )}
                            ...
                            {leaderStock && (
                                <div className="flex items-center gap-1.5 mb-2 pb-2 border-b border-[var(--border-color)]/50">
                                    ...
                                    <span className="font-bold text-[var(--text-primary)] ml-auto">
                                        {formatTradingValue(leaderStock.tradingValue)}
                                    </span>
                                </div>
                            )}
                            ...
                            <div className="flex-1 space-y-1 overflow-hidden">
                                {top3Stocks.map((stock, i) => (
                                    <div key={i} className="flex items-center justify-between text-[10px]">
                                        <span className="text-[var(--text-tertiary)] truncate flex-1 mr-2">
                                            {stock.stockName}
                                        </span>
                                        <span
...
function MarketStatusBadge({ marketStatus }: { marketStatus: MarketStatusInfo | null }) {
    ...
    return (
        <div
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border ${style.bg} ${style.text} ${style.border}`}
        >
            {style.icon}
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-hide">
            ...
            {(themeCounts['other'] || 0) > 0 && (
                ...
                >
                    기타
                    <span
                        className={`${selectedCategory === 'other' ? 'text-white/70' : 'text-[var(--text-tertiary)]'}`}
                    >
                        {themeCounts['other']}
                    </span>
                </button>
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                ...
                <div className="bg-[var(--bg-primary)] rounded-2xl border border-[var(--border-color)] p-5 shadow-[var(--shadow-sm)]">
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        <h3 className="font-bold text-[var(--text-primary)]">상승 테마 TOP 5</h3>
                    </div>
                    <div className="space-y-2">
                        {top5Gainers.length === 0 ? (
                            <div className="text-sm text-[var(--text-tertiary)] py-6 text-center">
                                상승 테마가 없습니다
                            </div>
                        ) : (
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            <span className="text-lg font-bold text-[var(--rise-color)] w-6">
                                                {i + 1}
                                            </span>
                                            <div>
                                                <div className="flex items-center gap-1.5">
                                        ...
                                        <span className="font-bold text-[var(--rise-color)]">
                ...
                <div className="bg-[var(--bg-primary)] rounded-2xl border border-[var(--border-color)] p-5 shadow-[var(--shadow-sm)]">
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        <h3 className="font-bold text-[var(--text-primary)]">하락 테마 TOP 5</h3>
                    </div>
                    <div className="space-y-2">
                        {top5Losers.length === 0 ? (
                            <div className="text-sm text-[var(--text-tertiary)] py-6 text-center">
                            ...
                            top5Losers.map((theme, i) => {
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            <span className="text-lg font-bold text-[var(--fall-color)] w-6">
                                                {i + 1}
                                            </span>
                                            <div>
                                                <div className="flex items-center gap-1.5">
                                        ...
                                        <span className="font-bold text-[var(--fall-color)]">
                                            {theme.avgChangeRate.toFixed(2)}%
                                        </span>
                                    </div>
                                );
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                {themes.map((theme) => (
                    <ThemeFlipCard key={theme.name} theme={theme} onClick={() => onThemeClick(theme.name)} />
                ))}
            </div>
        </div>
    );
        ...
        <div className="min-h-screen bg-[var(--bg-secondary)]">
            ...
            <main className="p-6">
                {themesLoading ? (
                    ...
                    <>
                        {viewMode === 'dashboard' && (
                            <DashboardView themes={filteredThemes} onThemeClick={handleThemeClick} />
                        )}
                        {viewMode === 'heatmap' && (
                            <TreemapHeatmapView
