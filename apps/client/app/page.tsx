    ...
    return (
        <aside className="w-56 bg-white border-r border-slate-200 hidden lg:flex flex-col fixed h-full z-20">
            ...
            <nav className="flex-1 px-2 py-4 space-y-1">
                <Link
                    href="/"
                    className="flex items-center gap-2 px-3 py-2 rounded-md bg-indigo-50 text-indigo-600 border-l-2 border-indigo-500 transition-all text-sm font-medium"
                >
                    <Newspaper size={16} />
                    <span>뉴스 피드</span>
                </Link>
    ...
    return (
        <div className="flex min-h-screen bg-slate-50 text-slate-700 font-sans selection:bg-indigo-500/30">
            ...
            <main className="flex-1 lg:ml-56">
                {/* 헤더 */}
                <header className="h-14 border-b border-slate-200 flex items-center justify-between px-4 bg-white/90 backdrop-blur-md sticky top-0 z-10">
                        ...
                        <div className="flex items-center bg-slate-100 rounded-lg p-0.5">
                            ...
                            >
                                ...
                                {analyzedNews.length > 0 && (
                                    <span className="text-indigo-600">{analyzedNews.length}</span>
                                )}
                            </button>
                        </div>
                ...
                <div className="flex h-[calc(100vh-56px)]">
                    {/* 왼쪽: 뉴스 테이블 */}
                    <div className="flex-1 min-w-0 p-4 border-r border-slate-200 overflow-hidden">
                        {isAnalyzedLoading && activeTab === 'analyzed' ? (
                            ...
                            <div className="flex flex-col items-center justify-center h-full text-slate-400">
                                <Radio size={32} className="mb-3 text-slate-300" />
                                <p className="text-sm">
                                    {activeTab === 'realtime'
                                        ? '실시간 뉴스를 기다리는 중...'
                                        : '분석된 뉴스가 없습니다'}
                    ...
                    {/* 오른쪽: 상세 패널 */}
                    <div className="w-[400px] shrink-0 bg-white">
                        <NewsDetailPanel
                            news={displayedNews}
