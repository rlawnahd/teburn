        ...
        <div onClick={onStockClick} className={`relative group border-b border-[var(--border-color)] hover:bg-[var(--bg-tertiary)] transition-colors cursor-pointer ${flashClass}`}>
            ...
            <div className="relative flex items-center py-2 px-3 md:px-4 gap-2 md:gap-3">
                ...
                <div className="text-right hidden sm:block w-20">
                    <span className="text-base text-[var(--text-secondary)]">{stock.currentPrice.toLocaleString()}</span>
                </div>

                <div className="w-16 md:w-18 text-right flex-shrink-0">
                    <span className={`text-base font-semibold ${stock.changeRate > 0 ? 'text-[var(--rise-color)]' : stock.changeRate < 0 ? 'text-[var(--fall-color)]' : 'text-[var(--text-tertiary)]'}`}>
                        {stock.changeRate > 0 ? '+' : ''}{stock.changeRate.toFixed(2)}%
                    </span>
                </div>

                <div className="w-14 md:w-16 text-right flex-shrink-0">
                    <span className="text-sm text-[var(--text-secondary)]">{formatTradingValue(stock.tradingValue)}</span>
                </div>
            </div>
        </div>
    );
...
export default function TopTradingView() {
    ...
    const { data, isLoading, error, refetch } = useQuery({
        ...
        queryFn: () => fetchLeadingStocks(minRate, 50),
        ...
        <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    <h2 className="text-base font-semibold text-[var(--text-primary)]">거래대금 TOP</h2>
